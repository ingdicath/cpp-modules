...
Bureaucrat::Bureaucrat(const std::string &name, short grade) : _name(name), _grade(grade) {
	...
	std::cout << GREEN "Bureaucrat '" << _name << "' was created." RESET << std::endl;
}
...
void Bureaucrat::executeForm(AForm const &form) const {
	try {
		form.execute(*this);
		std::cout << CYAN_L << _name << RESET " executed " CYAN_L << form.getName() << RESET << std::endl;
	} catch (AForm::FormNotSignedException &e) {
		std::cout << ORANGE << _name << RESET " coudn't executed " ORANGE
				  << form.getName() << RESET " because " << ORANGE << e.what() << RESET << std::endl;
